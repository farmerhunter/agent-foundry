...
| `check collaboration readiness for this repo` | `检查这个 repo 的 collaboration readiness` | Read labels, routing config, Execution Contracts, Testing Contracts, issue/PR routing, and optional Project mirror state. Reports findings and dry-run repair ideas only.<br>读取 labels、routing config、Execution Contracts、Testing Contracts、issue/PR routing 和可选 Project mirror state；只报告 findings 和 dry-run repair ideas。 |
| `prepare this repo for multi-agent collaboration` | `为这个 repo 准备 multi-agent collaboration` | Start the new-project setup checklist: role labels, routing template, Project mirror options, contracts, and readiness audit. Raw helper commands remain secondary/debug details.<br>启动新项目 setup checklist：role labels、routing template、Project mirror options、contracts 和 readiness audit。Raw helper commands 仍是 secondary/debug details。 |
| `audit existing collaboration setup` | `审计现有 collaboration setup` | Review drift in an existing project and return safe next steps. The audit is read-only and does not repair GitHub or Project v2 state.<br>review 现有项目的 drift，并返回安全 next steps。Audit 是 read-only，不 repair GitHub 或 Project v2 state。 |
