...
| `onboard this existing project into V2 Local Orchestration as a read-only trial` | `把这个 existing project 作为只读 trial 接入 V2 Local Orchestration` | Start an adopter-project onboarding packet: confirm Base default vs explicit Local Orchestration trial, read durable issues/PRs/comments/branch evidence, write only temp evidence, fall back from missing `stage:*` labels to explicit issue/PR selection, and name Human-owned decisions and stop/defer conditions. |
