...
## Legacy and developer/operator reference

Historical V2/JSONL trials, `--ledger-root` examples, controlled dogfood,
fake Project execution, migration diagnostics and detailed recovery evidence
are **not** end-user onboarding. They remain non-authoritative developer/operator
reference in [the GitHub collaboration helper workflow](../workflows/github-collaboration-helper.md).
They never create a fallback or dual-write authority. For a supported daily path,
start with SQLite onboarding/status/action/recovery above and use the safe next
action returned by its owner API.
