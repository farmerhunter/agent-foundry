...
1. 一次工作中出现的教训，先只是 candidate lesson。它需要经过 dedupe，判断是否已经被已有 practice 覆盖。如果确实有普遍性，才进入 canonical practice。Practice 不是一段感想，而是一个能改变未来 agent 行为的规则。
2. 多个 practices 可以被打包成 asset，比如 architecture-design、practice-harvester、agent-collaboration。Practice 和 asset 的权威记录应该留在 selected User Vault。
3. Asset 再被转换成不同 agent 的 adapter，进入 Codex、Claude Code、Hermes 或 ChatGPT 的运行环境。这确保了最重要的边界 - Generated 和 Runtime 只是从 canonical records 派生出来的运行态表达。Approved practice / asset 不能被某个 agent runtime 的本地副本反向改写。
4. 最后，真实使用会产生 evidence，review 再根据 evidence 判断它是否有用、是否过时、是否应该修改。
5. Capability pack 位于这条链路的分发层。它不是另一套 source of truth，而是把一组已经 review 过的 practices、assets、docs 和 payload 组织成可预览、可部署、可更新、可停用的能力包。
...
## 我对 human-agent collaboration 的思考
我认为AI coding 的瓶颈在迅速移动。

- 早期瓶颈主要是模型能力。人全程扑进去补模型能力的短板：解释背景、纠正错误、重写方案、盯住输出质量。（Question：你的MBTI类型是什么？）
- 中期瓶颈开始转向协作本身。在高阶的模型和合适harness的环境中，模型已经能大量产出，且输出物质量有了稳定预期。但人不一定能有效吸收这些产出。真正稀缺的东西变成了人的注意力：人需要做有价值的判断、设定边界、提出方向、检查风险，而不是被拖进每一个细节里做人工调度员。
- 未来瓶颈会进一步转向能力成长和能力转移。随着Agent的自成长能力的构建和提升，AI 在长期使用中逐渐学会做得更稳，人的判断被沉淀成可迁移的 capability。人可以在这个协作舞台上逐步退后，只在方向、边界、风险和验收处介入。

**【观点】： Human-agent collaboration 的要诀，是让人的注意力待在正确的位置，保持控制感和实际控制力，并追求长期效率最优。**

保护**注意力**，首先要减少低价值打断。低风险、可验证、重复性的动作，不应该反复要求人确认。相反，系统应该自动完成、留下证据，并在异常时停下来。人真正需要看的，是那些无法靠测试或规则完全判断的地方：目标是否仍然正确，边界是否被突破，风险是否值得接受，用户价值是否成立。

但保护注意力不等于让人失去控制。
- 好的**控制感**来自几个明确条件：人能看见当前状态，知道谁在负责，理解下一步会发生什么，清楚哪些动作被禁止，并且确信出错后可以停下、回退或重新接管。
- 真正的**控制力**来自可见的状态机、清晰的权限边界、可审查的证据和可靠的恢复路径。机械的HITL（如确认弹窗）不能替代系统性的控制设计。

这也是 role-based collaboration 有价值的原因。Architect、Implementer、Reviewer、Coordinator 不同责任分开。设计、实现、审查、协调，本来就是不同的思考姿态。混在一个连续对话里，agent 很容易一边产出一边替自己验收；拆成角色后，人可以更清楚地看到：现在是在做判断、做实现、做独立审查，还是维护流程连续性。

长期价值也要换一种算法。一次任务简单粗暴做快一点当然爽，但更大的收益来自减少重复。少重复解释项目背景，少重复恢复上下文，少重复提醒同样的边界，少重复修同类错误，少重复协调谁该接手下一步。这些节省的不只是 token，也是人的注意力和信任成本。

一个好的协作系统，应该让下一次工作比这一次更容易进入状态。过去的判断能被复用，过去的错误能变成 guardrail，过去的流程能变成更短的 handoff。这样 AI 的产出速度才不会把人压垮，而是逐步把人从低价值细节里释放出来。
...
这些东西如果结构足够好，就不只是“别人写过的一段提示词”，而是可部署的能力包。一个优秀工程师、研究者、投资人、设计师、运营者的经验，可能被部分蒸馏成 agent 可以学习和执行的资产。**人可以拿出来交换/分享/售卖的不只是知识，还有工作方式，习惯套路（甚至包括“品味”？）。**
